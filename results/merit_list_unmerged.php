...
// Load subjects for class & exam (UNMERGED)
...
while ($row = mysqli_fetch_assoc($subjects_q)) {
    // Include parts only if full marks configured in exam
...
// Order subjects: compulsory first, optional later, group-aware
...
if ($students_q) {
    while ($stu = mysqli_fetch_assoc($students_q)) {
        ...
        foreach ($display_subjects as $sub) {
            ...
            if (empty($assignedByStudent[$stu['student_id']][$sidSub])) { continue; }
            ...
            $failedThis = !$isPass;
            if ($failedThis && $is_compulsory) { $fail_count++; $failed_subject_codes[] = (string)$subject_code; }

            $gpa = $failedThis ? 0.00 : subjectGPA($sub_total, $sub['max_marks']);
        ...
        $all_students[] = [
            ...
            'row' => [
                ...
                'gpa' => $final_gpa,
                'status' => $status,
                'fail_count' => ($fail_count > 0 ? $fail_count : ''),
...
<head>
<meta charset="UTF-8">
<title>Merit List (Unmerged)</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<style>
    th, td { vertical-align: middle !important; font-size: 13px; padding: 3px; }
...
<?php foreach ($ranked_students as $i => $stu): $serial = $i + 1; $merit_pos = $i + 1; $r = $stu['row']; ?>
    <tr>
        ...
        <td><?= htmlspecialchars((string)$r['gpa']) ?></td>
        <td><?= $r['status'] ?></td>
        <td><?= htmlspecialchars((string)$r['fail_count']) ?></td>
        <td><?= htmlspecialchars($r['failed_codes']) ?></td>
    </tr>
<?php endforeach; ?>
    </tbody>
  </table>
