...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action'] ?? '')==='save_duties'){
    ...
    $isValidDate = (bool)preg_match('~^\d{4}-\d{2}-\d{2}$~',$duty_date) && $duty_date!==NULL && $duty_date!=='0000-00-00' && $plan_id>0;
    $isMappedDate = false;
    if ($isValidDate) {
      $allowed = [];
      $sqlDatesPost = "SELECT DISTINCT es.exam_date AS d FROM seat_plan_exams spe JOIN exam_subjects es ON es.exam_id=spe.exam_id WHERE spe.plan_id=".(int)$plan_id." AND es.exam_date IS NOT NULL";
